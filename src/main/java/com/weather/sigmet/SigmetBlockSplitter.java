...
final class SigmetBlockSplitter {
    ...
    /** 匹配美国 CONVECTIVE SIGMET 头行，如 "CONVECTIVE SIGMET 65W" */
    private static final Pattern CONVECTIVE_HEADER = Pattern.compile("^CONVECTIVE\\s+SIGMET\\s+\\d+[EWC]?", Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);
    ...
    private List<SigmetBlock> splitBySigmetHeaders(HazardType hazard, String raw) {
        ...
        for (int i = 0; i < rawLines.length; i++) {
            if (SIGMET_HEADER.matcher(rawLines[i]).find() || CONVECTIVE_HEADER.matcher(rawLines[i]).find()) {
                headerIndices.add(i);
            }
        }
